...
A verified project backup contains the same authoring scope and exclusions declared by portable v1 plus strict recovery metadata; large ZIP bodies are not stored in SQLite. Browser-native player saves remain separate `cyoa.native-player-save/v1` records in that browser's IndexedDB and are not inside project backups. Credentials, environment settings, raw reasoning, machine paths, temporary files, browser saves, and active SQLite WAL/journal files are excluded. Recovery is local and provider-independent; it is not automatic cloud backup or sync.
...
Recorded AI usage is read from persisted attempts and immutable aggregate review/proposal records without contacting a provider. Attempts and provider requests are reported separately because one attempt can make an initial generation request plus a structural-repair request. When durable repair evidence proves the call count, it is exact; incomplete historical evidence is labeled **partial** or **unknown**, never guessed as zero. Token and cost facts remain the exact persisted aggregates by workflow/provider/model, excluding rollups and candidate mirrors so totals are not double-counted. A recorded cost is shown only when the historical record contains one. Missing cost remains **unknown**; a total with both known and unknown components is labeled **partial**. The app never silently estimates old costs or reprices old usage with current model prices.
