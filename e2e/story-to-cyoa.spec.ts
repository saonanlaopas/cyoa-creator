...
test("repair planning and bounded proposals stay explicit and immutable across a 300-passage project", async ({ page, request }) => {
  ...
  await expect(page.locator(".repair-generation-job .eyebrow")).toHaveText("completed", { timeout: 30_000 });
  ...
  const applicationReview = page.getByLabel("Repair application review");
  await applicationReview.getByRole("checkbox").check();
  await applicationReview.getByRole("button", { name: "Preview selected repair" }).click();
  await expect(applicationReview).toContainText("Accepted prose stays locked and unchanged.");
  await expect(applicationReview).toContainText(/Provider calls\s*0/);
  await expect(applicationReview).toContainText("Targeted verification");
  const applicationPreviewFingerprint = await applicationReview.locator(".repair-application-preview > strong").textContent();
  ...
  const proposalId = (await (await request.get(`/api/long-form/projects/${projectId}/repair/proposals`)).json()).items[0].id as string;
  const proposalGroupId = (await (await request.get(`/api/long-form/projects/${projectId}/repair/proposals/${proposalId}`)).json()).groups[0].id as string;
  const duplicate = await request.post(`/api/long-form/projects/${projectId}/repair/proposals/${proposalId}/apply`, {
    data: { selectedGroupIds: [proposalGroupId], previewFingerprint: applicationPreviewFingerprint },
  });
  ...
  await page.locator(".repair-proposal-history button").filter({ hasText: "ai-assisted" }).first().click();
  await expect(page.getByLabel("Repair application review")).toContainText("historical");
