...
The passage-plan stage now also contains the first bounded-production control: a persisted generation-plan and job kernel. Actual AI passage proposals, drafting, simulation, and publishing remain later checkpoints.
...
Checkpoint 4A-1 executes only a deterministic offline lifecycle kernel. **Start offline kernel** demonstrates durable unit scheduling, status, cancellation, independent retry, and restart recovery without generating or applying passage-plan content. Completed units are preserved. Failed or interrupted units remain inspectable and retryable within the fixed attempt ceiling. No browser request can raise backend unit, context, output, or retry limits. Real bounded context construction, structured passage output, consolidation, proposal review, and transactional proposal application arrive in later Foundation 4A checkpoints.
