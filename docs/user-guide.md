...
For a selected passage, **Passage draft** now provides the Foundation 4B-1 manual corpus architecture. **Save new candidate version** stores prose and its author note as a new immutable passage-specific draft; it never overwrites prior versions and never calls a provider. Each version retains the exact passage-plan version and approved brief, bible, route, ending, and mechanics versions it was based on. **Draft history** can restore an older version as a new candidate while preserving every intervening version. Candidate prose is separate from accepted prose, and the underlying lifecycle can represent accepted, reviewed, locked, and stale versions; the polished acceptance/review workflow remains a later checkpoint.

Material changes to a passage's purpose, summary, word target, route or ownership references, characters, relationships, locations, facts, threads, choices, terminal/ending state, or drafting notes mark its existing drafts stale without deleting their prose. Choice changes also stale their source passage, and approved upstream planning changes stale drafts that retain the older dependency. Cosmetic title, tag, planning-status, unresolved-question, and position edits do not stale prose, and an unrelated passage edit does not stale the selected passage. Manual edits, restores, and applied Foundation 4A proposals use this same policy. Word counts use Unicode letter/number runs, retaining internal apostrophes and hyphens as one word; the panel reports current passage words plus project accepted and remaining words.

Open **Drafting-plan architecture** to preview or save a deterministic, provider-free drafting plan for the selected passage. The saved plan captures exact immutable dependencies, bounded units, estimates, diagnostics placeholders, model identity, and an authorization fingerprint. The current backend limits are 8 passages per unit, 100 units per plan, 48,000 estimated input tokens per unit, 2,500 output tokens per passage, 12,000 output tokens per unit, 3 attempts per unit, and 96,000 serialized candidate bytes. Browser requests cannot raise these ceilings. Foundation 4B-1 includes durable authorization, cancellation, independent retry, and restart recovery for lifecycle testing, but it deliberately builds no full prose context, calls no prose provider, and generates no prose; actual bounded prose generation begins in Foundation 4B-2.
